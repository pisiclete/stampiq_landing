...
export const Logo = ({ height = 32, mono = false, markOnly = false }) => {
  const iconColor = mono ? 'currentColor' : SIQ.green;
  const wordColor = mono ? 'currentColor' : SIQ.fg;
  return (
    <svg viewBox={markOnly ? '0 0 174.29 174.29' : '0 0 705.14 174.29'} height={height} style={{ display: 'block' }} aria-label="StampIQ">
      <g>
        <g fill={wordColor}>
...
export const Pill = ({ variant = 'green', children, style, className }) => {
  const t = useT();
...
export const PhoneMockup = ({ src, w = 280, h = 580, style }) => (
  <div style={{
...
export const GradientHeadline = ({ children, style, className }) => (
  <h2 className={className} style={{
    fontSize: 'clamp(32px, 4.5vw, 48px)', fontWeight: 800, lineHeight: 1.1, margin: 0,
    background: `linear-gradient(135deg, ${SIQ.fg} 0%, ${SIQ.green} 100%)`,
    WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent',
...
export const SigiVisionMark = ({ light = false, size = 18 }) => (
  <span style={{
...
export const SigiPose = ({ pose = 'waving', size = 200, style, flip = false, className }) => (
...
export const SigiSilhouette = ({ pose = 'waving', size = 600, color = 'rgba(255,255,255,0.10)', style, flip = false, className }) => (
